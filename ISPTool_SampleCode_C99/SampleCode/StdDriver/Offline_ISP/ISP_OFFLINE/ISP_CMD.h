...
struct sISP_COMMAND
{
    ...
    ErrNo(*ISPUpdateFlash)(uint32_t data_addr, uint32_t in_startaddr, uint32_t in_file_totallen);
    void (*ISPOpen)(void);
};


extern void init_ISP_command(void);
extern void ISPOpen(struct sISP_COMMAND *ISP_COMMAND);
...
extern int Updated_Target_Flash(struct sISP_COMMAND *gISP_COMMAND, uint32_t data_addr, uint32_t in_startaddr, uint32_t in_file_totallen);
